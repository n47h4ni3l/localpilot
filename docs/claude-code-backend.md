...
The wrapper fixes `cwd` to the candidate, uses `shell=False`, restricted and bare modes, disables session persistence, ignores external MCP servers and customizations, and makes permission prompts fail closed. Read tools are confined to the candidate, while Edit and Write permissions are generated as exact rules for the grounded path allowlist. Web tools, subagents, skills, hidden evaluation data, LocalPilot data, parent traversal, package managers, destructive commands, shell nesting, and Git mutation commands are denied. Only repository reads, grounded edits, narrow Git inspection, Python compilation, and existing pytest/unittest commands are allowed.
...
After every process run, LocalPilot derives changed paths from Git and rejects ungrounded paths, protected tests, deletions, links, disallowed file types, oversized files, invalid UTF-8, and secret-like assignments. LocalPilot then reruns the repository's bounded pytest or unittest suite in the candidate with a token-free environment and retains only pass/fail, exit code, and an output digest. This independent evidence also makes Ollama-backed Claude completion robust when the CLI returns its documented envelope but the custom model does not reproduce the requested inner JSON exactly. A passing candidate is still independently reviewed by LocalPilot and then by CI and a human. Human merge remains mandatory.
