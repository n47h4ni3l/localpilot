...
The `qlora_v1.yaml` pins model revisions and all adapter, data, optimization, validation, checkpoint, seed, and resource settings. The first approved run was interrupted before its epoch-one checkpoint and had no recoverable trainer state. The current owner-approved eager run uses an early step-5 checkpoint followed by saves every 50 steps, retaining three checkpoints. Validation remains at 3,704 steps. The shorter saves are recovery points, not extra validation runs. Real save/resume checks passed on the target machine before this configuration was approved.

The Windows host launcher (`training/scripts/start_guarded_adapter.ps1 -Mode Resume`) leaves the trainer settings and checkpoint identity unchanged. Policy v3 defaults to `-MemoryPolicy MonitorOnly`: it records host available RAM, Windows commit headroom, the top working-set/private-memory processes, available per-process GPU-memory counters, and the latest verified checkpoint without automatically stopping a viable training run. This is intentional after repeated sustained-threshold stops failed to establish that host RAM pressure was the cause of the interruption. `CriticalOnly` is available as an explicit opt-in for only the extreme thresholds, and `Protective` preserves the former v2 sustained-stop behavior when that trade-off is wanted.
...
The approved trainer itself still writes the early step-5 checkpoint and then a complete checkpoint every 50 optimizer steps, retaining three verified checkpoints. This cadence is already much shorter than the original epoch-scale save and is deliberately left unchanged here so existing approved-run identity and resumable checkpoints are not invalidated by an operational monitoring change.

On the owner's PC the on-demand `LocalPilot Adapter Training Guard` and `LocalPilot Training Monitor` Windows scheduled tasks launch independently of the chat app. The launcher refuses to start while the LocalPilot broker/runtime worker, Ollama, or the LocalPilot Background Worker are active. Consequently normal LocalPilot/SystemSense sampling is not running during an approved guarded training session and cannot be the periodic host-RAM consumer seen by this monitor. Use the launcher after closing competing GPU workloads. These are not automatic restart loops. Closing the monitor does not stop training, but signing out, rebooting, sleeping, a driver failure, or genuine resource exhaustion can still interrupt it.
