...
Final synthesis remains a high-reasoning generative step, including when durable memory guided the research. A deterministic postcondition then checks asserted repository paths, configuration fields, Python symbols and direct call relationships against the live tree, checks a small structured set of high-impact information-flow, safety-authority and promotion-lifecycle contracts, and requires claim-specific observations for consequential PC-state claims such as storage or the active power plan. Those repository contract anchors are themselves validated by `RepositoryGroundingValidator`. Proposed/future work, external APIs and ordinary non-repository answers are excluded. Passing prose is preserved; there is no generic verifier rewrite. Only a failed claim triggers a bounded correction, at most twice, before unsupported prose is withheld.
