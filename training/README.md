...
The Windows host guard (`training/scripts/start_guarded_adapter.ps1 -Mode Resume`) leaves those training settings and checkpoint identities unchanged. Policy v2 warns below 2.5 GiB available RAM or commit headroom. It stops for RAM below 1 GiB continuously for 30 seconds, or commit headroom below 2.5 GiB for 15 seconds. It stops immediately below 0.25 GiB RAM or 1 GiB commit headroom. Recovery resets each pressure timer independently; elapsed time uses a monotonic clock. These are operational margins, not guarantees against a driver or OS freeze. Each sample is saved to an ignored `training/reports/adapter_v1_*.memory.jsonl` file, and status includes the policy and exact stop reason. Failed monitoring also requests a stop rather than leaving an unguarded run. A safety stop resumes from the latest complete checkpoint; it does not promise an emergency save under memory pressure.

On the owner's PC the on-demand `LocalPilot Adapter Training Guard` and `LocalPilot Training Monitor` Windows scheduled tasks launch independently of the chat app. Use the guard task to resume only after checking there is no existing trainer and competing GPU workloads are closed. These are not automatic restart loops. Closing the monitor does not stop training, but signing out, rebooting, or sleeping can interrupt it.
