...
Current LocalPilot repository/architecture answers receive one bounded low-reasoning authority review after synthesis. A deterministic postcondition then checks asserted repository paths, configuration fields, Python symbols and direct call relationships against the live tree, and checks a small structured set of high-impact information-flow, safety-authority and promotion-lifecycle contracts. Those contract anchors are themselves validated by `RepositoryGroundingValidator`. Proposed/future work, external APIs and ordinary non-repository answers are excluded. The live index is cached only while a path/content-digest fingerprint remains unchanged, and this postcondition consumes no operator tool rounds. Unsupported claims are corrected at most twice and otherwise withheld.
