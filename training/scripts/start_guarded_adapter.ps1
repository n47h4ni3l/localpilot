...
function Get-GpuProcessMemory {
    try {
        ...
        $counters = Get-Counter -Counter $counterNames -ErrorAction Stop
        $byProcessId = @{}

        foreach ($sample in $counters.CounterSamples) {
            $path = [string]$sample.Path
            if ($sample.Status -ne 0 -or $path -notmatch 'pid_(\d+)') {
                continue
            }

            $processId = [int]$Matches[1]
            if (-not $byProcessId.ContainsKey($processId)) {
                $byProcessId[$processId] = [ordered]@{
                    pid = $processId
                    name = $null
                    dedicated_gib = 0.0
                    shared_gib = 0.0
            ...
            if ($path -like '*dedicated usage') {
                $byProcessId[$processId].dedicated_gib += $sample.CookedValue / 1GB
            }
            elseif ($path -like '*shared usage') {
                $byProcessId[$processId].shared_gib += $sample.CookedValue / 1GB
            }
        }

        foreach ($processId in @($byProcessId.Keys)) {
            try {
                $byProcessId[$processId].name = (Get-Process -Id $processId -ErrorAction Stop).ProcessName
            }
            catch {
                $byProcessId[$processId].name = $null
            }

            $byProcessId[$processId].dedicated_gib = [math]::Round($byProcessId[$processId].dedicated_gib, 3)
            $byProcessId[$processId].shared_gib = [math]::Round($byProcessId[$processId].shared_gib, 3)
        }

        return @(
            $byProcessId.Values |
            Sort-Object { [double]$_.dedicated_gib + [double]$_.shared_gib } -Descending |
            Select-Object -First 15
        )
    }
    catch {
...
function Get-WindowsMemoryBreakdown {
    try {
        ...
        function Convert-BytesToGiB($value) {
        ...
        return [ordered]@{
            ...
            cache_gib = Convert-BytesToGiB $memory.CacheBytes
            system_cache_resident_gib = Convert-BytesToGiB $memory.SystemCacheResidentBytes
            paged_pool_gib = Convert-BytesToGiB $memory.PoolPagedBytes
            nonpaged_pool_gib = Convert-BytesToGiB $memory.PoolNonpagedBytes
            modified_page_list_gib = Convert-BytesToGiB $memory.ModifiedPageListBytes
            standby_core_gib = Convert-BytesToGiB $memory.StandbyCacheCoreBytes
            standby_normal_gib = Convert-BytesToGiB $memory.StandbyCacheNormalPriorityBytes
            standby_reserve_gib = Convert-BytesToGiB $memory.StandbyCacheReserveBytes
            committed_gib = Convert-BytesToGiB $memory.CommittedBytes
            commit_limit_gib = Convert-BytesToGiB $memory.CommitLimit
        }
    }
    catch {
        return [ordered]@{
...
function Get-WslMemoryState {
    try {
        $meminfo = @(wsl.exe -d LocalPilot-Training -- timeout 5s cat /proc/meminfo 2>$null)
        if ($LASTEXITCODE -ne 0 -or $meminfo.Count -eq 0) {
            return [ordered]@{
                error = 'WSL meminfo unavailable.'
                top_processes = @()
            }
        }
        ...
        $processLines = @(
            wsl.exe -d LocalPilot-Training -- timeout 5s ps -eo pid=,ppid=,comm=,rss=,vsz=,pcpu= --sort=-rss 2>$null |
            Select-Object -First 15
        )
        ...
        foreach ($line in $processLines) {
            ...
            try {
                $rssKiB = [double]$parts[3]
                $vszKiB = [double]$parts[4]
                $cpu = [double]$parts[5]
            }
            catch {
                continue
...
function Write-GuardStatus {
    ...
    [ordered]@{
        ...
        latest_complete_checkpoint_step = if ($null -ne $LatestCheckpoint) {
            $LatestCheckpoint.step
        } else {
            $null
        }
        latest_complete_checkpoint_path = if ($null -ne $LatestCheckpoint) {
...
$checkpointActivity = Get-CheckpointActivity
$topMemory = @()
$gpuMemory = @()
...
$trainingProcess = Start-Process @startProcessArgs
...
while (-not $trainingProcess.HasExited) {
    try {
        ...
        $limit = @(
            $counters.CounterSamples |
            Where-Object Path -Like '*commit limit'
        )
        ...
        [ordered]@{
            ...
            checkpoint_activity = if ($needDetail) { $checkpointActivity } else { $null }
            latest_complete_checkpoint_step = if ($null -ne $latestCheckpoint) {
                $latestCheckpoint.step
            } else {
                $null
...
Write-GuardStatus -State $finalState -AvailableGiB $available -CommitHeadroomGiB $headroom -ExitCode $trainingProcess.ExitCode -LatestCheckpoint $latestCheckpoint
    ...
    $entry = $latest[0]
    $files = @(
        Get-ChildItem -LiteralPath $entry.path -File -Recurse -ErrorAction SilentlyContinue
    )
    ...
    return [ordered]@{
        step = $entry.step
        complete = (Test-Path -LiteralPath $entry.marker -PathType Leaf)
        bytes_gib = [math]::Round(([double]$bytes) / 1GB, 3)
        file_count = $files.Count
        modified_at_utc = $entry.modified.ToString('o')
    }
}
