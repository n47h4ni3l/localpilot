...
The `qlora_v1.yaml` pins model revisions and all adapter, data, optimization, validation, checkpoint, seed, and resource settings. The first long-running owner-observed attempt used embedding offload to CPU and progressed for more than 1,000 optimizer steps before an external interruption. The later 20 GiB/no-offload recovery configuration repeatedly failed with GPU out-of-memory errors within a few resumed steps even while the v3 host monitor reported healthy Windows RAM/commit headroom. The recovery specification therefore restores `offload_embeddings=true` / `cpu_offload=embeddings_only`, keeps eager mode and adapter-only resumable checkpoints, and writes to a fresh output directory. It is deliberately `proposed` until a new target-machine dry-run passes and the owner explicitly approves that exact changed specification.

The Windows host launcher (`training/scripts/start_guarded_adapter.ps1`) follows the output directory in the active config rather than a hard-coded historical run name. For this recovery topology it requires embedding offload, at least 24 GiB of configured WSL memory and 24 GiB of WSL swap; the dry-run still verifies the actual guest RAM and storage before model load. Policy v3 defaults to `-MemoryPolicy MonitorOnly`: it records host available RAM, Windows commit headroom, the top working-set/private-memory processes, available per-process GPU-memory counters, and the latest verified checkpoint without automatically stopping a viable training run. `CriticalOnly` and `Protective` remain explicit opt-in intervention modes.
...
The recovery trainer still writes the early step-5 checkpoint and then a complete checkpoint every 50 optimizer steps, retaining three verified checkpoints. The previous no-offload checkpoint-250 belongs to a different training identity and must not be reused or relabeled. The new output path intentionally starts a fresh identity after the recovery dry-run/approval gate.

On the owner's PC the on-demand `LocalPilot Adapter Training Guard` and `LocalPilot Training Monitor` Windows scheduled tasks launch independently of the chat app. The launcher refuses to start while the LocalPilot broker/runtime worker, Ollama, or the LocalPilot Background Worker are active. It also clears inherited `PYTORCH_CUDA_ALLOC_CONF` / `PYTORCH_ALLOC_CONF` overrides so failed allocator experiments cannot contaminate a later recovery run. Use the launcher after closing competing GPU workloads. Closing the monitor does not stop training, but signing out, rebooting, sleeping, a driver failure, or genuine resource exhaustion can still interrupt it.
